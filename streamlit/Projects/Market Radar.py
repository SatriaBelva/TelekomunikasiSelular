...
with colMap :
    ...
    with st.container(border=True) :
        st.header('Penggunaan Layanan Telekomunikasi')
        pendidikanGraph, pendidikanMetric = st.columns([0.7,0.3])
        with pendidikanGraph :
            chart_data = pd.DataFrame(np.random.randn(20, 3), columns=["a", "b", "c"])
            st.area_chart(chart_data)
        with pendidikanMetric :
            st.metric(label=f"Jumlah Penduduk Tamatan SD", value=f'1000', delta=f'39.11%', delta_color='normal')
